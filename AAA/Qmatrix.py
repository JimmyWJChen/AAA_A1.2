import numpy as np
# Shortens theodorsen constants significantly
...
class StructuralSection:
    def __init__(self, a: float, b: float, c: float, m: float, S: float, S_β: float, I_α: float, I_αβ: float, I_β: float, C_h: float, C_α: float, C_β: float, K_h: float, K_α: float, K_β: float, K_h7: float = 0, A_h: float = 0, eq_lin: bool = False) -> None:
        ...
        self.K_h7 = K_h7    # [N/m^7]   Non-linear heave stiffness

        # Assumed response of non-linear DOF
        self.A_h = A_h      # [m]       Heave amplitude

        self.M_s = self.mass_matrix()
        self.C_s = self.damping_matrix()
        self.K_s = self.stiffness_matrix()

        # if using equivalent linearisation, update K_s
        self.eq_lin = eq_lin
        if self.eq_lin:
            self.K_s = self.update_linear_stiffness_matrix()
    ...
    @property
    def K_h7_equivalent(self) -> float:
        """
        Calculates the equivalent linearisation of the non-linear stiffness.

        returns
        -------
        K_h7_equivalent: float
            The equivalent linearisation of the non-linear heave stiffness based on a first order FS.
        """
        return 35/64*self.A_h**6 * self.K_h7
    ...
    def update_linear_stiffness_matrix(self) -> np.ndarray:
        """
        Update the linear structural stiffness matrix by adding the equivalent non-linear stiffness.

        returns
        -------
        K: float
            The updated structural stiffness matrix with the linearised non-linear stiffness contribution.
        """
        K = self.stiffness_matrix()
        K[0, 0] += self.K_h7_equivalent
        return K
    ...
    def compute_coupled_undamped_eigenmodes(self):
        ...
        """
        self.ωs, self.U = scipy.linalg.eigh(
            a=self.K_s, b=self.M_s)  # symmetric matrices, eigh is faster and sorted than eig.
    ...
    def compute_uncoupled_undamped_eigenmodes(self):
        ...
        # symmetric matrices, eigh is faster and sorted than eig.
        self.ωs, self.U = scipy.linalg.eigh(a=self.K_s, b=M_s_uncoupled)
        self.ωs = np.sqrt(self.ωs)  # Solving for ω^2, units [rad/s]
        return self.ωs, self.U
...
class AeroelasticSection():
    def __init__(self, structural_section: StructuralSection, ρ: float, v: float) -> None:
        ...
        self.T_2 = c*(1 - c**2) + c*acos(c)**2 - \
            sqrt(1 - c**2)*(c**2 + 1)*acos(c)
        self.T_3 = c*sqrt(1 - c**2)*(2*c**2 + 7)*acos(c)/4 - \
            (1 / 8 - c**2 / 8)*(5*c**2 + 4) + (-c**2 + -1 / 8)*acos(c)**2
        self.T_4 = - acos(c) + c*sqrt(1 - c**2)
        self.T_5 = -(1 - c**2) - (acos(c))**2 + 2 * c * sqrt(1-c**2) * acos(c)
        self.T_6 = c*(1 - c**2) + c*acos(c)**2 - \
            sqrt(1 - c**2)*(c**2 + 1)*acos(c)
        ...
        self.T_13 = -c*sqrt(1 - c**2)*(2*c**2 + 7)/16 - (-self.a + c)*(
            c*acos(c) - sqrt(1 - c**2)*(c**2 + 2)/3)/2 - (-c**2 + -1 / 8)*acos(c)/2
    ...
    def circulatory_matrices(self, Ck: float) -> tuple[np.ndarray, np.ndarray]:
        """
        Generates the circulatory aerodynamic matrices according to appendix A
        """
        b = self.b
        a = self.a
        ...
        C_a = self.ρ * self.v * b * Ck * np.array([
            ...
            [2*np.pi*b*(a + (1 / 2)), 2*np.pi*b**2*((1 / 2) - a) *
             (a + (1 / 2)), self.T_11*b**2*(a + (1 / 2))],
    ...
    def non_circulatory_matrices(self):
        ...
        M_a_nc = - self.ρ * self.b**2 * np.array([
            ...
            [-np.pi*self.a*self.b, np.pi*self.b**2 *
                (self.a**2 + (1 / 8)), -self.b**2*(self.T_1*(-self.a + self.c) + self.T_7)],
            [-self.T_1*self.b, 2*self.T_13*self.b**2, -self.T_3*self.b**2/np.pi]])

        C_a_nc = -self.ρ * self.v * self.b**2 * np.array([
            [0, np.pi, -self.T_4],
            [0, np.pi*self.b*((1 / 2) - self.a), self.b*(self.T_1 +
                                                         self.T_11/2 - self.T_4*(-self.a + self.c) - self.T_8)],
